...
export const competitors: readonly Competitor[] = [
  {
    ...
    lede: 'MSW is the de-facto request-mocking library for JavaScript apps. It intercepts fetch/XHR in the browser via a Service Worker and in Node via request-level hooks, and it is excellent for component and unit tests where you want a handful of endpoints stubbed with a couple of lines of code. Carbon plays a different role: it is a real HTTP server that models the whole API and holds state across requests, which is where MSW starts to feel hand-rolled.',
    rows: [
      {
        label: 'State persistence across requests',
        carbon: 'yes',
        competitor: 'partial',
        note: 'Possible by hand — you write the store yourself inside a handler.',
      },
      { label: 'Snapshots / rewind (state journal)', carbon: 'yes', competitor: 'no' },
      {
        label: 'Chaos / failure injection',
        carbon: 'yes',
        competitor: 'partial',
        note: 'You can throw or return a 500 in a handler; no first-class latency/chaos presets.',
      },
      {
        label: 'Multi-format ingest (OpenAPI, GraphQL, HAR, Postman, gRPC)',
        carbon: 'yes',
        competitor: 'partial',
        note: 'GraphQL and REST handlers ship in-box; other formats need adapters.',
      },
  ...
  },
  {
    ...
    lede: 'WireMock is the incumbent in the JVM world — a mature HTTP mock server with stub mappings, request matching, and record/playback. It has scenarios for stateful flows, which is a step above simple response mocking. Carbon takes a different approach: instead of hand-writing stub mappings and scenario transitions, you point it at a spec and get a stateful server for free.',
    rows: [
      {
        label: 'State persistence across requests',
        carbon: 'yes',
        competitor: 'partial',
        note: 'Yes via "scenarios" — you define the states and transitions by hand.',
      },
      { label: 'Snapshots / rewind (state journal)', carbon: 'yes', competitor: 'no' },
      {
        label: 'Chaos / failure injection',
        carbon: 'yes',
        competitor: 'yes',
        note: 'Fault injection, fixed delays, and random delays are supported.',
      },
      {
        label: 'Multi-format ingest (OpenAPI, GraphQL, HAR, Postman, gRPC)',
        carbon: 'yes',
        competitor: 'partial',
        note: 'OpenAPI import exists via extensions; gRPC support is separate. Check WireMock docs.',
      },
      { label: 'Local-first (runs on your laptop)', carbon: 'yes', competitor: 'yes' },
      {
        label: 'No hosted-service dependency',
        carbon: 'yes',
        competitor: 'yes',
        note: 'Self-hostable; WireMock Cloud is a separate paid option.',
      },
      {
        label: 'Framework glue (vitest, jest, playwright)',
        carbon: 'yes',
        competitor: 'partial',
        note: 'JVM test-framework integrations are first-class; JS glue is thinner.',
      },
      { label: 'Drift detection vs real API', carbon: 'yes', competitor: 'no' },
    ],
    whenPickCompetitor: [
  ...
  },
  {
    ...
    lede: 'Prism turns an OpenAPI spec into a mock server in one command. It is the fastest way to get a spec-driven mock on a port. But Prism is deliberately stateless — POST /pets returns an example, and GET /pets afterwards does not include the pet you just created. Carbon starts from the same spec and models the state so those two calls agree.',
    rows: [
      {
        label: 'State persistence across requests',
        carbon: 'yes',
        competitor: 'no',
        note: 'Prism responds from spec examples; it does not track created resources.',
      },
      { label: 'Snapshots / rewind (state journal)', carbon: 'yes', competitor: 'no' },
      {
        label: 'Chaos / failure injection',
        carbon: 'yes',
        competitor: 'partial',
        note: 'Can return any spec-documented status via the "Prefer" header; not a chaos preset system.',
      },
      {
        label: 'Multi-format ingest (OpenAPI, GraphQL, HAR, Postman, gRPC)',
        carbon: 'yes',
        competitor: 'partial',
        note: 'OpenAPI 2/3 only.',
      },
      { label: 'Local-first (runs on your laptop)', carbon: 'yes', competitor: 'yes' },
      { label: 'No hosted-service dependency', carbon: 'yes', competitor: 'yes' },
      {
        ...
        carbon: 'yes',
        competitor: 'partial',
        note: 'CLI is easy to boot from a test, but no first-party JS test bindings.',
      },
      {
        label: 'Drift detection vs real API',
        carbon: 'yes',
        competitor: 'partial',
        note: 'Prism has spec validation of live traffic; not the same as diffing shapes across runs.',
      },
    ],
    whenPickCompetitor: [
  ...
  },
  {
    ...
    rows: [
      ...
      { label: 'Snapshots / rewind (state journal)', carbon: 'yes', competitor: 'no' },
      {
        ...
        carbon: 'yes',
        competitor: 'partial',
        note: 'Per-route latency and custom status codes; no unified chaos preset.',
      },
      {
        label: 'Multi-format ingest (OpenAPI, GraphQL, HAR, Postman, gRPC)',
        carbon: 'yes',
        competitor: 'partial',
        note: 'OpenAPI import supported; other formats: check Mockoon docs.',
      },
      { label: 'Local-first (runs on your laptop)', carbon: 'yes', competitor: 'yes' },
      {
        label: 'No hosted-service dependency',
        carbon: 'yes',
        competitor: 'yes',
        note: 'Mockoon Cloud is optional.',
      },
      {
        label: 'Framework glue (vitest, jest, playwright)',
        carbon: 'yes',
        competitor: 'partial',
        note: 'A CLI exists for CI, but there are no first-party test-framework bindings.',
      },
      { label: 'Drift detection vs real API', carbon: 'yes', competitor: 'no' },
    ],
    whenPickCompetitor: [
  ...
  },
  {
    ...
    rows: [
      ...
      { label: 'Snapshots / rewind (state journal)', carbon: 'yes', competitor: 'no' },
      {
        ...
        carbon: 'yes',
        competitor: 'partial',
        note: 'You can save error examples and match them via headers; no chaos presets.',
      },
      {
        label: 'Multi-format ingest (OpenAPI, GraphQL, HAR, Postman, gRPC)',
        carbon: 'yes',
        competitor: 'partial',
        note: 'Native to Postman collections; other formats via import.',
      },
      {
        label: 'Local-first (runs on your laptop)',
        carbon: 'yes',
        competitor: 'no',
        note: 'Mocks are served from Postman-hosted infrastructure.',
      },
