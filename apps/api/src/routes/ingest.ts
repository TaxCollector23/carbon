  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (body.async) {
        ...
        // Detach from the request lifecycle: setImmediate lets the 202 flush
        // before the potentially long ingestion pipeline starts. When the
        // BullMQ worker is running out-of-process this should become an
        // enqueue call instead — tracked as follow-up.
        const jobId = job.id;
        setImmediate(() => {
          runIngestJob(ctx, jobId, { ...body, projectSlug: project.storageSlug }).catch((err) => {
            ctx.logger.error('ingest.job_unhandled', {
              jobId,
