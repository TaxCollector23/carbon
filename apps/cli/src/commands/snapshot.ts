  ...
  subCommands: {
    save: defineCommand({
      ...
      async run({ args }) {
        ...
        await ensureDir();
        try {
          const res = await fetch(`${args.runtime}/__carbon/state/snapshot`, { method: 'POST' });
          if (!res.ok) throw new Error(`runtime returned ${res.status}`);
          const snapshot = await res.json();
          const path = join(SNAP_DIR, `${args.name}.json`);
          await writeFile(path, JSON.stringify(snapshot, null, 2), 'utf8');
          ui.success(`Snapshot ${ui.code(args.name)} saved`);
          ui.step('Path', path);
        } catch (err) {
          ui.error(`Could not reach runtime at ${args.runtime}: ${(err as Error).message}`);
    ...
    load: defineCommand({
      ...
      async run({ args }) {
        ...
        const path = join(SNAP_DIR, `${args.name}.json`);
        if (!existsSync(path)) {
          ui.error(`Unknown snapshot: ${args.name}`);
          process.exitCode = 1;
          return;
        }
        const snapshot = JSON.parse(await readFile(path, 'utf8'));
        const res = await fetch(`${args.runtime}/__carbon/state/restore`, {
          method: 'POST',
    ...
    delete: defineCommand({
      ...
      async run({ args }) {
        ...
        await unlink(path);
        ui.success(`Deleted snapshot ${ui.code(args.name)}`);
      },
    }),
  },
});
